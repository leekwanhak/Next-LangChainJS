// 호출주소: http://localhost:3001/api/mixbot
...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      const botType = req.body.botType as BotType; //사용자 챗봇타입 추출
      ...
      //Step3-3: Embeding/Embeding 수치데이터 저장 과정: 1. Splitting된 문서내 단어들을 임베딩(벡터화처리)처리해서 메모리벡터저장소에 저장하기
      //MemoryVectorStore.fromDocuments(임베딩된문서,사용할 임베딩모델 처리기);
      //지정한 임베딩모델을 통해 chunk data를 개별 vetor 수치화하고 수치화된 데이터를
      //지정한 Vector 전용 저장소에 저장한다.
      //   const vectorStore = await MemoryVectorStore.fromDocuments(
      //     docs,
      //     new OpenAIEmbeddings()
      //   );
      //2. PineCone 클라우드 벡터 DB에 저장하기 - 지정한 Cloud Pinecone Vector Index 전용 저장소에 저장한다.
      // 사용할 임베딩 모델 객체 생성하기
